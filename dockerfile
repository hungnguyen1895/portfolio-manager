...
COPY --from=compile /usr/src/mymaven/target/portfolio-0.0.1-SNAPSHOT.jar portfolio-app.jar
COPY src/main/resources/application-docker.properties application.properties
EXPOSE 8080
ENV JAVA_OPTS=""
ENTRYPOINT [ "sh", "-c", "java $JAVA_OPTS -Djava.security.egd=file:/dev/urandom -jar /portfolio-app.jar" ]
